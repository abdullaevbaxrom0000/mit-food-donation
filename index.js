...
try {
  ...
  if (userCheck.rows.length === 0) {
    await pool.query(
      ...
      [id, username || `${first_name} ${last_name}`, '+998991234567', photo_url || null] // Телефон захардкожен, можно потом заменить
    );
    ...
    console.log('Аватарка обновлена:', { userId: id, avatar_url: photo_url });
